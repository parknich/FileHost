<template>
  ...
  <div class="dashboard">
    <!-- Sidebar with added Settings button -->
    <aside class="sidebar">
      <h2>Files</h2>
      <ul>
      ...
      <button class="settings-button" @click="showSettingsMenu = true">
        Settings
      </button>
    </aside>
    ...
    <main class="content">
      <div class="header">
        <h1>My Files</h1>
        <button class="upload-button" @click="showUploadMenu = true">
          Upload File
        </button>
      </div>
  ...
  <!-- Settings Menu -->
  <transition name="fade">
    <div v-if="showSettingsMenu" class="settings-menu" @click.self="showSettingsMenu = false">
      <div class="settings-menu-content">
        <h2>Settings</h2>
        <div>
          <h3>API Key Management</h3>
          <button @click="generateApiKey">Generate API Key</button>
          <p v-if="apiKey">Your API Key: {{ apiKey }}</p>
        </div>
      </div>
    </div>
  </transition>
...
const generateApiKey = async () => {
  try {
    const response = await api.post('/api/auth/generateApiKey');
